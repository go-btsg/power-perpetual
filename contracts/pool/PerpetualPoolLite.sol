...
contract PerpetualPoolLite is IPerpetualPoolLite {
    ...
    function _liquidate(address liquidator, address account) internal _lock_ {
        ...
        if (position.volume != 0) {
            ...
            netEquity +=
                (((position.volume * _symbol.price) / ONE) * _protocolParameters.symbolMultiplier()) /
                ONE -
                position.cost;
        }
        ...
        if (curBlockNumber > preBlockNumber) {
            if (_symbol.tradersNetVolume != 0) {
                int256 ratePerBlock = (((((((((_symbol.tradersNetVolume * _symbol.price) / ONE) *
                    _symbol.price) / ONE) * _protocolParameters.symbolMultiplier()) / ONE) *
                    _protocolParameters.symbolMultiplier()) / ONE) *
                    _protocolParameters.symbolFundingRateCoefficient()) / totalDynamicEquity;
        ...
        returns (IPTokenLite.Position memory position, int256 margin)
    {
        ...
        if (position.volume != 0) {
            ...
            funding += (((position.volume * position.volume) / ONE) * delta) / ONE;
